...
/**
 * @brief Checks if the vector passes both the ordering and difference criteria by removing at most one element.
 * @param numbers A vector of integers to check.
 * @return true if the vector can be made valid by removing at most one element, false otherwise.
 */
...
int main() {
  ...
  while (std::getline(input, line)) {
    ...
    if (isOrdered(numbers) && hasValidDifferences(numbers) || isValidWithOneRemoval(numbers)) {
      sum += 1;
    }
    numbers.clear();
  }
